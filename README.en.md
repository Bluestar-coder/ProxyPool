...
A desktop proxy pool manager built with PyQt6. Crawl proxies from multiple sources, validate connectivity, run speed tests, and serve the pool via local services.
...
- **Multi-source crawling** — FOFA, Hunter, Quake, and free public proxy sites
...
Keys are stored in the **system keychain** (macOS Keychain / Windows Credential Manager) — never in source files or the database. Enter them once in **Settings → Crawlers**.
...
| Data | Location |
|------|----------|
| Database (proxies, config) | macOS: `~/Library/Application Support/ProxyPool/` / Windows: `%APPDATA%\ProxyPool\` |
